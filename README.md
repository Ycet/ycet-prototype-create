...
- 对零散想法或不完整需求调用 `brainstorming-solo` 完善需求；阶段一只处理产品需求，UI 风格、色彩、字体、视觉参考与 UI Skill 均推迟到阶段二。
...
- PNG/JPG 等整页图片必须先生成只显示完整原图的 `pages/**/*.html` 承载页与 `index.html`，禁止把图片解构或重绘为页面元素。

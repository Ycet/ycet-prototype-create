...
画布交互固定为：普通鼠标滚轮滚动当前 HTML 页面，`Ctrl+鼠标滚轮`以指针为中心缩放，中键按住进行二维平移。画布左下角必须常驻这两项操作提示，并设置为不接收指针事件。进入工作台时“选择元素”默认不激活，预览运行时不得显示悬浮框、已选框、元素名称或批注入口；用户主动点击后才进入选择模式。关闭“选择元素”后必须隐藏全部选择覆盖层，再次开启后需重新点击元素才显示选区。批注入口必须位于选区框外的右上角或右下角并与右边缘对齐；批注内容浮层靠近 HTML 可视区边缘时必须自动左右翻转并上下避让，不能被 iframe 裁切。同一元素已有批注时隐藏新增入口，删除该元素批注或清空当前文件批注后重新显示。

预览运行时必须上报页面真实内容宽度，工作台据此扩展 iframe 逻辑宽度，避免固定预览宽度裁切页面；预览高度使用中央画布可用高度，超出部分由 HTML 页面自身纵向滚动。X/Y 输入展示元素当前视口坐标，写回时必须计算“新坐标 − 选中时坐标”的差量并叠加到元素原有 `left/top`；即使元素位于非零起点的定位容器中，数值增加 1 也只能移动 1 CSS 像素。静态元素需先转为相对定位，不能写入无效偏移。

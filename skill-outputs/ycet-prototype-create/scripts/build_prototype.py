...
def render(model, root):
    ...
    meta={'schemaVersion':1,'skillVersion':'4.0.1','artifactVersion':model.get('artifactVersion',1),'type':kind,'productPort':model.get('productPort',''),'frame':frame,'initial':initial,'pages':[{'id':p['id'],'label':p.get('label',p['id'])} for p in pages]}

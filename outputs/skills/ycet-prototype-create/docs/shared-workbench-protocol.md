...
本协议供功能一至五共享。工作台负责本地预览、元素选择、批注、预览级草稿、变更包和结果展示；Agent 仍是唯一可以在用户点击“发送给 AI”后修改源文件的一方。
...
- 左栏只读展示已登记文件，分组名称左侧显示本地文件夹图标，组内文件相对文件夹标题向右缩进；刷新按钮扫描 `prototype/` 根级及约定子目录并补入尚未登记的项目 HTML，不删除、移动、重命名或重新排序任何磁盘文件。
...
预览路由必须以项目 `prototype/` 为 URL 根保留原 HTML 的目录语义：根级 HTML、`pages/`、`previews/`、`runtime-pages/`、设备框架及其嵌套 iframe 中的相对 `src`、`srcset`、`href`、CSS `url()` 和模块路径，都必须按源文件所在目录解析。响应内注入的 `<base>` 只能映射到当前 HTML 在预览路由中的等价目录，不能把 `pages/*.html` 的 `../assets/images/*` 重定向到缺失文件 ID 的路径；不得为修复预览而改写磁盘 HTML。

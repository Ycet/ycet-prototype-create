...
- 左栏只读展示已登记文件，分组名称左侧显示本地文件夹图标；刷新按钮扫描 `prototype/` 根级及约定子目录并补入尚未登记的项目 HTML，不删除、移动、重命名或重新排序任何磁盘文件。
...
画布交互固定为：普通鼠标滚轮滚动当前 HTML 页面，`Ctrl+鼠标滚轮`以指针为中心缩放，中键按住进行二维平移。画布左下角必须常驻这两项操作提示，并设置为不接收指针事件。关闭“选择元素”后必须隐藏悬浮框、已选框、元素名称和批注入口；再次开启后需重新点击元素才显示选区。批注入口必须位于选区框外的右上角或右下角并与右边缘对齐；批注内容浮层靠近 HTML 可视区边缘时必须自动左右翻转并上下避让，不能被 iframe 裁切。同一元素已有批注时隐藏新增入口，删除该元素批注或清空当前文件批注后重新显示。

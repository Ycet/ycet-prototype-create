...
- 属性输入框与文本字段的每次修改在失焦（或回车）时提交一次，逐键输入不产生多个草稿操作与撤销步；点击发送、清空或撤回前会先自动提交尚未失焦的修改。
- “撤回”撤销上一步右侧边栏操作：样式修改（一次点击撤销一次用户操作的整批属性，宽高联动同时恢复宽度与高度、四角圆角整体恢复）与图片替换（恢复上一次替换或回到原始图片）；不覆盖文本内容、批注与 `同步 pages`；输入框尚未失焦的修改在点击撤回时先提交为一步，再被整体撤回；撤回后预览与右侧栏输入同步恢复（含嵌套 iframe 内元素的撤回）；可连续回退且不设步数上限，发送成功或“清空修改”后历史清空；历史仅存浏览器内存，随会话失效。

...
3. 把全部页面写成直接 DOM fragment，CSS 用 :scope 定位页面根，JS 在 root 参数内查询，跨页使用 navigate(pageId)。资源先归档再内联；展示壳按 presentation-design.md 复用已确认的 shellTheme／presentation，自动生成标题、说明、分组导航与页面卡片。

...
9. 用户可点击顶部 Power 图标关闭当前工作台进程；有未发送草稿时先确认丢失风险，已发送或正在执行的 Agent 请求继续保留。
...
- 关闭服务只清空未发送浏览器草稿，不删除已发送请求、执行结果或撤回事务，也不终止正在运行的 Agent CLI。
...
- 每组按文件名升序展示；网页端不提供拖拽排序或其他手动排序入口。
...
- 交接弹窗展示请求 ID、文件数、操作数、动态状态和执行指令；剪贴板失败时不得误报已复制。
...
7. Agent 写入项目内 `EditLog`、生成最近一次回滚事务并回传结果；工作台进入对应终态并刷新成功文件。
...
14. Power 按钮能在二次确认后优雅停止当前实例并清理 `server.json`；重新 `ensure` 可启动新实例，已发送请求和结果仍可恢复。
...
实现时必须遵守：源 HTML 在“发送给 AI”前不可被编辑器改写；外部文件只能在用户明确要求时由 Agent 通过 CLI 登记；左栏不提供新增、移出、删除、手动分组或排序入口；未发送草稿不可跨重启恢复；`同步 pages` 不可破坏功能三的运行时交互；回滚只能恢复最近一次成功批次且需摘要校验；功能五打包前必须阻止影响输入的未发送草稿，打包期间不得改写任何既有 `prototype/` 输入或 `EditLog.md`，只允许新增一个递增离线单文件。
